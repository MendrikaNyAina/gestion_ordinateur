<%@page contentType="text/html;charset=UTF-8" language="java" %>
     <%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
          <%@taglib prefix="h" uri="http://www.front.org/html.tags" %>
               <%@taglib prefix="g" uri="http://www.front.org/page.tags" %>
                    <%@page import="java.util.ArrayList, app.code.model.*" %>
                         <!DOCTYPE html>
                         <html lang="en">
                         <g:Head title="Login Admin" isSweetAlert="true"></g:Head>

                         <body>
                              <div class="auth-wrapper">
                                   <div class="auth-content text-center">
                                        <h:Card classe="borderless">
                                             <h:Row classe="align-items-center">
                                                  <h:Col md="12">
                                                       <h:CardBody>
                                                            <h4 class="mb-3 f-w-400">Login Admin</h4>
                                                            <hr>
                                                            <form action="/login/admin" method="post" id="form">
                                                                 <h:Input id="username" name="username" placeholder="username"
                                                                      value="admin">
                                                                 </h:Input>
                                                                 <h:Input id="password" name="password" type="password"
                                                                      value="admin">
                                                                 </h:Input>
                                                                 <h:Button color="primary" classe="btn-block mb-4"
                                                                      type="submit">Login
                                                                 </h:Button>
                                                                 <hr>

                                                            </form>
                                                            <p class="mb-0 text-muted"><a href="/store/login" class="f-w-400">Connection magasin</a></p>

                                                       </h:CardBody>
                                                  </h:Col>
                                             </h:Row>
                                        </h:Card>
                                   </div>
                              </div>
                              <script src="/assets/js/ajaxG.js"></script>
                              <script type="text/javascript">
                                   const url = "/admin/login";
                                   const form = document.querySelector("#form");
                                   form.addEventListener("submit", function (e) {
                                        e.preventDefault();

                                        const username = document.querySelector("#username").value;
                                        const password = document.querySelector("#password").value;

                                        const data = {
                                             username: username,
                                             password: password
                                        };
                                        console.log(data);
                                        call(url, "POST", data, function (response) {
                                             console.log(response);
                                             Swal.fire({
                                                  title: 'Login reussi',
                                                  icon: 'success',
                                                  confirmButtonText: 'OK'
                                             });
                                             window.location.href = "/admin/stocks";


                                        }, function (xhr, status, error) {
                                             console.log(xhr);
                                             Swal.fire({
                                                  title: 'Erreur',
                                                  text: xhr.responseText,
                                                  icon: 'error',
                                                  confirmButtonText: 'OK'
                                             });
                                        }
                                        );
                                   });

                              </script>
                         </body>

                         </html>
